...
namespace JPMorrow.Revit.Tools.ConduitFittings
{
	...
	readonly public struct Fitting
	{
		...
		public string GetAngleString(ModelInfo info) => RMeasure.AngleFromDouble(info, Angle);
		public string GetDiameterString(ModelInfo info) => RMeasure.LengthFromDbl(info, Diameter);
		...
		{
			double cvt_ang = RMeasure.AngleDbl(info, angle);
			double cvt_dia = RMeasure.LengthDbl(info, diameter);
